...
class GalgeRuntimeApp {
  constructor() {
    ...
    this.makuhariFrameInterval = 1000 / 20;
  ...
  resolveDreamEffectProfile() {
    ...
    return date === "2026-04-25" || scenarioName.startsWith("2026-04-25_")
      ? "makuhari"
      : "default";
  }
  ...
  restoreAtmosphereSnapshot(snapshot) {
    ...
    const bgImage = hasSceneBg
      ? this.ensureSceneBgImageElement()
      : document.getElementById("scene-bg-img");
    if (!bgImage) {
      return;
    }
    if (!hasSceneBg) {
  ...
  bindEvents() {
    ...
    document.addEventListener("click", (event) => {
      if (
        ...
        event.target.closest("#end-screen button")
      ) {
        return;
      }
  ...
  initMakuhariDream(width, height) {
    ...
    const particleCount = Math.min(64, Math.max(38, Math.round(area / 24000)));
    const lineCount = Math.min(12, Math.max(7, Math.round(area / 130000)));
  ...
  resetMakuhariParticle(particle = {}, width = this.$canvas.width, height = this.$canvas.height) {
    ...
    particle.tunnelSpeed = 0.0024 + Math.random() * 0.0048;
    return particle;
  }
  ...
  drawKongou = () => {
    ...
    if (this.isMakuhariDreamEffect()) {
      ...
      if (now - this.makuhariLastFrame < this.makuhariFrameInterval) {
        this.animId = requestAnimationFrame(this.drawKongou);
        return;
      }
      this.makuhariLastFrame = now;
    }
  ...
  getMakuhariChoiceTargets() {
    const now = performance.now();
    if (now - this._choiceTargetLast < 280) {
      return this._choiceTargetCache;
    }

    this._choiceTargetLast = now;
    ...
    this._choiceTargetCache = Array.from(this.$choiceContainer.querySelectorAll(".choice-btn"))
      .filter((button) => button.dataset.locked !== "true" && !button.disabled)
      .map((button, index) => {
  ...
  drawMakuhariDream() {
    ...
    const ctx = this.ctx;
    const now = performance.now();
    const dt = Math.min(66, Math.max(16, this.makuhariLastPaint ? now - this.makuhariLastPaint : this.makuhariFrameInterval));
    this.makuhariLastPaint = now;
  ...
  updateAndDrawMakuhariCausality(time, dt, targets) {
    ...
    for (let index = 0; index < this.makuhariParticles.length; index += 1) {
      ...
      const pull = (0.00006 + particle.z * 0.000044) * target.weight;
      ...
      particle.vx += Math.sin(time * 0.9 + particle.seed) * 0.005 * dtScale;
      particle.vy += Math.cos(time * 0.82 + particle.seed * 0.7) * 0.004 * dtScale;
      particle.vx *= 0.965;
      particle.vy *= 0.965;
